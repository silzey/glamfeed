...
import { Store, Star, ArrowLeft, Coins } from 'lucide-react';
...
export default function CoinShopPage() {
  ...
  useEffect(() => {
    getProducts().then(prods => {
      // For the coin shop, let's filter for products that have "Coins" in their price
      const coinProducts = prods.map(p => ({...p, price: `${Math.floor(parseInt(p.price) / 10)} Coins`}));
    ...
    <div className="flex min-h-screen w-full flex-col bg-black text-white">
      ...
      <main className="container mx-auto max-w-5xl px-4 pt-20 sm:pt-24 flex-1 pb-16 md:pb-24">
        ...
        <div className="flex items-center gap-4 mb-8">
            ...
            />
            <div>
                <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">Coin Shop</h1>
                <p className="text-md sm:text-lg text-white/70">
                Spend your coins on exclusive products.
                </p>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {products.map(product => {
                ...
                return (
                    <Link href={`/coinshop/${product.id}`} key={product.id} className="block group">
